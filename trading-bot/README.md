...
Each area has its own colour: gold for the desk, cyan for Markets, violet for Calls and plans, sky blue for Is it working?, coral for You and learning, lilac for Under the hood. A breadcrumb over every page names the area in that colour and shows the area's other pages as one-click pills. The More panel has a search box (type, then Enter) and your recent pages. Emerald and rose still mean up and down only.
...
| ⚡ **Scalper** | Costs first, then liquidity, the news clock, the spread and the tape |
| 🎯 **The Call** | Up or down over the next 15 minutes, the readings behind it, and the honest score |
