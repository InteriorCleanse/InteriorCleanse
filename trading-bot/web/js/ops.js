...
const VIEWS = [['overview', 'Overview'], ['firstfill', 'First fill'], ['market', 'Market data'], ['paper', 'Paper engine'], ['research', 'Research & learning'], ['db', 'Database & integrity'], ['reconcile', 'Reconciliation'], ['soak', 'Soak'], ['log', 'Log & alerts']]
let view = 'overview'
...
const stagePill = (s) => `<span class="ev-st ${s === 'PASS' ? 'large' : s === 'FAIL' ? 'dev' : s === 'WAITING' ? 'early' : 'none'}">${esc(s)}</span>`
const checkPill = (s) => `<span class="ev-st ${s === 'PASS' ? 'large' : s === 'FAIL' ? 'dev' : s === 'MISSING' ? 'early' : 'none'}">${esc(s === 'NOT_APPLICABLE' ? 'N/A' : s)}</span>`
const iso = (ms) => (ms === null || ms === undefined ? '—' : `${when(ms)} (${new Date(ms).toISOString()})`)
const num = (x, d = 2) => (x === null || x === undefined ? '—' : Number(x).toFixed(d))

async function renderFirstFill(h) {
  const { data: f } = await getJson('/api/ops/first-fill')
  const chain = `<div class="ff-chain">${f.chain.map((s, i) => `<div class="ff-stage"><div class="ff-stage-n">${esc(s.name)}</div>${stagePill(s.status)}<div class="muted ff-stage-note">${esc(s.note)}</div></div>${i < f.chain.length - 1 ? '<div class="ff-arrow" aria-hidden="true">↓</div>' : ''}`).join('')}</div>`
  const head = `<div class="ev-heroes">${hero('FIRST PAPER FILL', f.status === 'WAITING' ? pill('NOT YET OBSERVED') : f.status === 'ACCEPTED' ? pill('ACCEPTED') : `<span class="ev-st dev">NOT ACCEPTED</span>`, f.status === 'WAITING' ? 'WAITING FOR FIRST REAL PAPER FILL' : f.note)}${hero('data source', `${pill(f.dataSource)} <span class="ev-st early">${esc(f.execution)}</span>`, f.dataSource === 'PAPER' ? 'live market data · simulated execution' : f.dataSource === 'MOCK' ? 'recorded feed · not market evidence' : 'the live flag is set')}${hero('durable verdict', f.durable.acceptedAt ? `accepted ${when(f.durable.acceptedAt)}` : '—', `${f.durable.regressed ? 'DISAGREEMENT since acceptance · ' : ''}human review ${f.durable.humanReviewed ? 'recorded' : 'open'} · last evaluated ${f.durable.lastEvaluatedAt ? when(f.durable.lastEvaluatedAt) : '—'}`)}</div>`
  if (f.status === 'WAITING') {
    return `${banner(h)}${head}${note('NOT ENOUGH REAL PAPER DATA — no simulated fill has occurred on this record. Every stage below is NOT YET OBSERVED; nothing is shown as passed for something that has not happened. When a strategy reaches its threshold inside a killzone and the risk engine approves, the order fills at the next candle open and this page fills in from the durable record.')}${card('THE CHAIN', chain, 'each stage turns PASS only when its checks pass on the record; WAITING when evidence is still to come; FAIL when the record disagrees with itself')}`
  }
  const p = f.position
  const groups = { A: 'A · Market data', B: 'B · Decision', C: 'C · Execution', D: 'D · State', E: 'E · Durable record', F: 'F · Checkpoint', G: 'G · Day report' }
  const checks = Object.entries(groups).map(([g, label]) => { const cs = f.checks.filter((c) => c.group === g); return `<tr><th colspan="4">${esc(label)}</th></tr>${cs.map((c) => `<tr><td class="muted">${esc(c.id)}</td><td>${checkPill(c.status)}</td><td>${esc(c.description)}${c.reason ? `<div class="loss">${esc(c.reason)}</div>` : ''}</td><td class="muted">${esc(c.evidence ?? '')}${c.recordId ? `<div>${esc(c.recordId)}</div>` : ''}</td></tr>`).join('')}` }).join('')
  const b = f.before
  const a = f.after
  const before = b ? kv([['decided at', iso(b.decidedAt)], ['signal id', esc(b.signalId ?? '—')], ['market', `${esc(b.symbol)} ${esc(b.interval)} · session ${esc(b.session)} · regime ${esc(b.regime)} · volatility ${esc(b.volatility ?? '—')}`], ['strategy', `${esc(b.strategyId ?? '—')} · ${esc(b.direction)}`], ['plan', `intended entry ${num(b.intendedEntry)} · stop ${num(b.stop)} · target ${num(b.target)} · ATR ${num(b.atr)} · quality ${b.quality}`], ['reason', esc(b.reason)], ['signal candle', b.signalCandle ? `${when(b.signalCandle.openTime)} O ${num(b.signalCandle.open)} H ${num(b.signalCandle.high)} L ${num(b.signalCandle.low)} C ${num(b.signalCandle.close)}` : '<span class="loss">not stored</span>'], ['fused decision', b.fused ? `${esc(b.fused.action ?? '—')} ${b.fused.score ?? '—'} · confirms ${b.fused.confirms.map(esc).join(', ') || '—'} · invalidates ${b.fused.invalidates.map(esc).join(', ') || '—'}` : '—'], ['contributors', b.fused && b.fused.contributors.length ? b.fused.contributors.map((c) => `${esc(c.id)} ${esc(c.action)} ${c.confidence}`).join(' · ') : '—'], ['evidence', b.evidence.length ? `<ul>${b.evidence.map((e) => `<li>${e.passed ? '✓' : '✗'} <b>${esc(e.step)}</b> — ${esc(e.detail)}</li>`).join('')}</ul>` : '<span class="loss">none recorded</span>'], ['risk checks', b.riskChecks.length ? `<ul>${b.riskChecks.map((r) => `<li>${r.passed ? '✓' : '✗'} <b>${esc(r.rule)}</b> — ${esc(r.detail)}</li>`).join('')}</ul>` : '<span class="loss">none recorded</span>'], ['risk verdict', b.riskVetoedBy ? `<span class="loss">vetoed by ${esc(b.riskVetoedBy)}</span>` : 'approved'], ['news', `${b.news.inBlackout === null ? '—' : b.news.inBlackout ? 'in blackout' : 'clear'} · minutes to blackout ${b.news.minutesToBlackout ?? '—'}`], ['order book at decision', `bid ${num(b.book.bid)} · ask ${num(b.book.ask)} · spread ${b.book.spreadPct === null ? '—' : num(b.book.spreadPct, 4) + '%'} · assumed slippage ${b.book.assumedSlippageBps ?? '—'} bp`], ['execution assumptions', `spread ${b.executionAssumptions.spreadBps} bp · slippage ${b.executionAssumptions.slippageBps} bp · latency ${b.executionAssumptions.latencyCandles} candle · max drift ${b.executionAssumptions.maxEntryDriftAtr} ATR`], ['versions', `engine ${esc(b.versions.engine ?? '—')} · features ${b.versions.features ?? '—'}`]]) : ''
  const after = a ? kv([['filled at', iso(a.filledAt)], ['simulated fill', `${num(a.entry)} × ${a.quantity} · risk ${num(a.riskUsd, 4)} · entry cost ${num(a.entryCostUsd, 6)} · latency ${a.latencyMs ?? '—'} ms`], ['fill candle', a.fillCandle ? `${when(a.fillCandle.openTime)} O ${num(a.fillCandle.open)} H ${num(a.fillCandle.high)} L ${num(a.fillCandle.low)} C ${num(a.fillCandle.close)}` : '<span class="loss">not stored</span>'], ['price path since fill', `${a.path.candles} candle(s) · high ${num(a.path.high)} · low ${num(a.path.low)} · last ${num(a.path.last)} · as of ${when(a.path.asOf)}`], ['position', esc(a.status)], ['exit', a.exit ? `${esc(a.exit.reason)} at ${num(a.exit.exit)} on ${when(a.exit.closedAt)} · ${a.exit.rMultiple === null ? '—' : num(a.exit.rMultiple) + 'R'} · pnl ${num(a.exit.pnlUsd)} · fees ${num(a.exit.feesUsd, 4)} · ${esc(a.exit.outcome ?? '—')} · ${a.exit.candlesHeld ?? '—'} candles` : 'not yet — position open'], ['excursions', `MAE ${num(a.excursions.mae)} · MFE ${num(a.excursions.mfe)} · reconciled ${a.excursions.reconciledAt ? when(a.excursions.reconciledAt) : 'not yet'}`], ['reconciliation', a.reconciliation ? `${pill(a.reconciliation.verdict)}${a.reconciliation.failed.length ? `<div class="loss">${a.reconciliation.failed.map(esc).join('; ')}</div>` : ''}${a.reconciliation.incomplete.length ? `<div class="muted">${a.reconciliation.incomplete.map(esc).join('; ')}</div>` : ''}` : 'runs at the close'], ['observer records', a.observationIds.length ? a.observationIds.map(esc).join(', ') : '<span class="loss">none yet</span>'], ['case studies', a.caseIds.length ? a.caseIds.map(esc).join(', ') : 'none'], ['post-mortem', a.postMortemIds.length ? a.postMortemIds.map(esc).join(', ') : 'not yet (written at the close)'], ['checkpoint', a.checkpoint ? `reached ${when(a.checkpoint.reachedAt)} · human review ${a.checkpoint.reviewed ? 'recorded' : 'open'}` : '<span class="loss">not yet</span>'], ['day report', a.dayReport ? `${esc(a.dayReport.dayKey)}: ${a.dayReport.signals} signal(s), ${a.dayReport.fills} fill(s), ${a.dayReport.closed} closed` : '—']]) : ''
  return `${banner(h)}${head}${note(`Record ${p.id} · ${p.direction} · ${p.strategyId ?? '—'} · decided ${when(p.openedAt)} · filled ${when(p.filledAt)} · ${p.status}${p.closedAt ? ` · closed ${when(p.closedAt)}` : ''}. Every figure below is SIMULATED EXECUTION on the recorded data.`)}
    ${card('THE CHAIN', chain)}
    ${card(`NO-HINDSIGHT AUDIT ${checkPill(f.hindsight.status)}`, `<div>${esc(f.hindsight.note)}</div>${f.hindsight.findings.length ? `<ul>${f.hindsight.findings.map((x) => `<li class="loss">${esc(x)}</li>`).join('')}</ul>` : ''}`, 'the existing case-study hindsight check, applied to the decision snapshot, the observer record and any linked case study')}
    ${card(`ACCEPTANCE CHECKS — ${f.summary.passed} passed · ${f.summary.failed} failed · ${f.summary.missing} missing · ${f.summary.notApplicable} n/a`, `<div class="scroll"><table>${checks}</table></div>`, 'PASS only from the durable record; MISSING is never a pass')}
    <div class="ff-split"><div class="card ff-before"><h2>${esc(b ? b.label : 'BEFORE / DECISION-TIME KNOWLEDGE')}</h2><div class="muted">Only what the engine had at ${b ? when(b.decidedAt) : '—'}. Nothing here was written after the decision.</div>${before}</div>
    <div class="card ff-after"><h2>${esc(a ? a.label : 'AFTER / EXECUTION AND OUTCOME')}</h2><div class="muted">What happened next, from the fill onward. This section never feeds the one above.</div>${after}</div></div>`
}
...
async function renderView() {
  ...
  try {
    ...
    out.innerHTML = view === 'firstfill' ? await renderFirstFill(h) : view === 'market' ? renderMarket(h) : view === 'paper' ? renderPaper(h) : view === 'research' ? renderResearch(h) : view === 'db' ? renderDb(h) : view === 'reconcile' ? await renderReconcile(h) : view === 'soak' ? renderSoak(h) : view === 'log' ? await renderLog(h) : renderOverview(h)
