...
test('the weekly review runs the reassessment, and the daily brief lists what is due', () => {
  ...
  const dry = rv.weeklyReview([], NOW)
  assert.deepEqual(dry.reassessment.staleItems, [], 'a read does not sweep')
  assert.equal(vault.getItem(old.id)!.status, 'CURRENT')
  const w = rv.weeklyReview([], NOW, { reassess: true })
