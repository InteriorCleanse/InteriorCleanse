...
export function recommendations(input: { paper: Dataset; backtest?: Dataset | null; drift?: DriftInput; now?: number; max?: number }): { items: Recommendation[]; note: string } {
  ...
  // Stale knowledge.
  const stale = [...listItems({ status: 'STALE' }), ...listItems({ status: 'REVIEW REQUIRED' }), ...listItems({ status: 'CONTRADICTED' })]
  if (stale.length) {
    out.push({ id: 'rec-stale', topic: 'stale knowledge', question: `Do ${stale.length} knowledge item(s) still hold?`, why: `${stale.length} item(s) are STALE, REVIEW REQUIRED or CONTRADICTED: ${stale.slice(0, 3).map((i) => i.title).join('; ')}${stale.length > 3 ? '; …' : ''}.`, dataExists: 'The items with their evidence counts and history; the paper record since they were written.', missing: 'A review decision (confirm, revise, retire) per item, and for directional items a re-run of the cohort they describe.', wouldAnswer: 'The knowledge review on the KNOWLEDGE tab; a cohort experiment for each directional item.', queueItemId: null, weight: 55 })
  }
  // Regime transitions.
