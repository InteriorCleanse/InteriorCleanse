...
function byAge(name: string, at: number | null, now: number, healthyMs: number, degradedMs: number, rule: string, detail: string): Mark {
  if (at === null) return { name, at, ageSec: null, status: 'STOPPED', rule, detail: `${detail} — never recorded.` }
  const age = now - at
...
export type HeartbeatInputs = { feed?: FeedHealth | null; now?: number; probe?: boolean }
...
export function heartbeat(input: HeartbeatInputs = {}): Heartbeat {
  ...
  const marketData = byAge('last market-data update', lastMarket >= 0 ? lastMarket : null, now, 2 * step, 6 * step, `HEALTHY ≤ 2 intervals, DEGRADED ≤ 6, STALE beyond (interval ${interval})`, feed ? `Prices via ${feed.mode}${feed.stream ? `, stream ${feed.stream.connected ? 'connected' : 'down'}, ${feed.stream.reconnects} reconnect(s)` : ''}.` : 'The feed did not report.')
  const lastCandleRow = store().lastCandles(config.symbol, interval, 1)[0]
  const candle = byAge('last candle', lastCandleRow ? lastCandleRow.closeTime : null, now, 2 * step, 6 * step, `HEALTHY ≤ 2 intervals after the close, DEGRADED ≤ 6, STALE beyond`, lastCandleRow ? `Last stored close ${new Date(lastCandleRow.closeTime).toISOString()} via ${lastCandleRow.source}.` : 'No candle stored.')
  const engineCycle = byAge('last engine cycle', ops.lastCycleAt, now, 2 * step, 6 * step, 'HEALTHY ≤ 2 intervals, DEGRADED ≤ 6, STALE beyond', `${ops.cycles} cycle(s) observed since the ops state began.`)
  const lastObs = listObservations({ limit: 1 })[0]
  const observerEvent = { ...byAge('last observer event', lastObs ? lastObs.time : null, now, 24 * 3_600_000, 3 * 86_400_000, 'HEALTHY ≤ 24 h, DEGRADED ≤ 3 days, STALE beyond — a quiet tape records nothing', lastObs ? `${lastObs.type} at ${new Date(lastObs.time).toISOString()}.` : 'No observation yet.') }
  ...
  const researchTick: Mark = ops.lastRun === null
    ? { name: 'last research tick', at: null, ageSec: null, status: 'STOPPED', rule: `HEALTHY ≤ 2 ticks (${Math.round(RESEARCH_TICK_MS / 60_000)} min each), DEGRADED ≤ 4, STOPPED when the next run is more than 4 ticks overdue`, detail: 'The research scheduler has not run yet.' }
  ...
  const persistence = byAge('last successful persistence', lastPersistence(), now, 2 * 60_000 + step, 6 * step, 'HEALTHY ≤ 2 min + 1 interval, DEGRADED ≤ 6 intervals, STALE beyond', 'A probe row written to the store and read back.')
  const healthCheck = byAge('last successful health check', lastHealthCheck(), now, 6 * step, 24 * step, 'HEALTHY ≤ 6 intervals, DEGRADED ≤ 24, STALE beyond', 'The last time the health API or the monitor evaluated the system.')
