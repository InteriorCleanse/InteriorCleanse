...
| `tests/commands.test.ts` | The command palette matcher: exact over prefix over word-start over substring over keyword, ties broken by the shorter label, keyword-only matches found, empty query returns all | 9 passing |
| `tests/owner-portfolio.test.ts` | The cross-company portfolio: real kept apart from demo, a demo never inflating a real total, revenue per currency never summed across, attention list for past-due/canceled real orgs, status and plan buckets, honest handling of a null (real) revenue | 11 passing |
| `tests/theme.test.ts` | The theme list and its default, and that any unknown stored value falls back to Arch rather than breaking the page | 4 passing |
| `tests/signals.test.ts` | The command-center signals layer: revenue-up-profit-down leads as critical, ad efficiency only fires past double, no divide by a null percentage, thin margin and refunds gated, overdue pipeline worded as not-revenue, severity ordering | 10 passing |
